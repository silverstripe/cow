...
class ComposerConstraintTest extends PHPUnit_Framework_TestCase
{
    public function testParseSemver() {
        ...
        $this->assertEquals('4.1.1-alpha1', $constraint->getFrom()->getValue());
        $this->assertEquals('4.99999.99999', $constraint->getTo()->getValue());

        $constraint = new ComposerConstraint('^4.1');
        $this->assertEquals('4.1.0-alpha1', $constraint->getFrom()->getValue());
        $this->assertEquals('4.99999.99999', $constraint->getTo()->getValue());

        $constraint = new ComposerConstraint('^4');
        $this->assertEquals('4.0.0-alpha1', $constraint->getFrom()->getValue());
        $this->assertEquals('4.99999.99999', $constraint->getTo()->getValue());
    }
    ...
    public function testParseLoose() {
        $constraint = new ComposerConstraint('~4.1.1');
        $this->assertEquals('4.1.1-alpha1', $constraint->getFrom()->getValue());
        $this->assertEquals('4.1.99999', $constraint->getTo()->getValue());
        ...
        $constraint = new ComposerConstraint('~4');
        $this->assertEquals('4.0.0-alpha1', $constraint->getFrom()->getValue());
        $this->assertEquals('99999.99999.99999', $constraint->getTo()->getValue());
    }

    public function testParseSelfVersion() {
        $constraint = new ComposerConstraint('self.version', new Version('4.1.1'));
        $this->assertEquals('4.1.1', $constraint->getFrom()->getValue());
        $this->assertEquals('4.1.1', $constraint->getTo()->getValue());
    }
