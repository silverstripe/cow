...
class CreateChangelog extends ReleaseStep
{
     ...
     * @var string
     */
    protected $autogeneratedDelimiter = "<!--- Changes below this line will be automatically regenerated -->";
    ...
    protected function storeChangelog(OutputInterface $output, ChangelogLibrary $changelogLibrary, $content)
    {
        ...
        if ($path) {
            ...
            $header = $this->getFileHeader($output, $version, $existingContent, $changelogTemplatePath);
     ...
     * @return string
     */
    protected function getFileHeader(OutputInterface $output, Version $version, $content, $templatePath = '')
    {
        // Generate new header if no file exists
        if (empty($content)) {
            return $this->renderFileHeader($output, $version, $templatePath);
        }
        ...
        $position = stripos($content, $this->autogeneratedDelimiter);
        if ($position === false) {
            $this->log(
                $output,
        ...
        return $content . $this->autogeneratedDelimiter;
    }

    protected function renderFileHeader(OutputInterface $output, Version $version, $templatePath = '')
    {
        ...
        $output = (new Template())->renderTemplateWithContext(
            $template,
